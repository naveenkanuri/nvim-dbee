...
local required_pattern_markers = {
  "^ARCH14_DRAWER_FILTER_SCOPED_%d+_SENTINEL_OK$",
...
local function evaluate(lines)
  ...
  for _, pattern in ipairs(required_pattern_markers) do
    local matched = false
    for label, values in pairs(markers) do
    ...
    if not matched then
      failures[#failures + 1] = "missing pattern " .. pattern
    end
  end
  ...
  return {
    ok = #failures == 0,
    failures = failures,
    checked = #required_true_markers + vim.tbl_count(advisory_markers) + #required_pattern_markers,
  }
end
...
local function selftest()
  ...
  lines[#lines + 1] = "ARCH14_DRAWER_FILTER_SCOPED_10_SENTINEL_OK=true"
  lines[#lines + 1] = "ARCH14_DRAWER_FILTER_MIXED_VISIBLE_AND_LAZY_SENTINEL_OK=true"
