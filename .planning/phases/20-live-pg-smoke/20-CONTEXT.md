...
**Gathered:** 2026-05-07
**Revised:** 2026-05-08 r5 narrow fold
**Status:** Ready for r6 plan-gate
...
- **PG20-44:** CI fetches enough history for the locked-helper range check and keeps Git outside sudo. The `live-pg20-smoke` checkout uses `actions/checkout@v4` with `fetch-depth: 0`, or an explicit `git fetch origin master:refs/remotes/origin/master` before `make live-pg-smoke`. The outer Makefile target, running as the checkout owner, assigns `base="$$(git merge-base HEAD origin/master)"` and fails closed without emitting `LIVE_PG20_LOCKED_HELPERS_UNTOUCHED_OK=true` if merge-base resolution fails. The outer target performs Git checks only before sudo. Runtime probe, SQL-shape preflight, live test, and rollup verification run inside `_live-pg-smoke-inner`, invoked directly by default or via `sudo -E make _live-pg-smoke-inner` when `LIVE_PG20_USE_SUDO=1`. `_live-pg-smoke-inner` must not invoke `git merge-base`, `git diff`, or any other Git command. The CI job must not call `sudo -E make live-pg-smoke`.
...
*Context revised: 2026-05-08 r5 narrow fold*
