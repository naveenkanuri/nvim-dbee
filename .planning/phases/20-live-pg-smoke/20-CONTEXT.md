...
**Gathered:** 2026-05-07
**Revised:** 2026-05-08 r6 narrow fold
**Status:** Ready for r7 plan-gate
...
- **PG20-25:** Keep `LIVE_PG20_HISTORICAL_UNNEST_NEGATIVE_OK` only for the exact historical broken FK SQL. The live test uses `github.com/lib/pq v1.10.9`, `var pqErr *pq.Error`, `require.ErrorAs(t, err, &pqErr)`, and `require.Equal(t, pq.ErrorCode("42883"), pqErr.Code)`. `database/sql` preserves the `*pq.Error` from `lib/pq`; any non-`42883` error class fails both negative markers. The Go test must log `LIVE_PG20_NEGATIVE_SQLSTATE_42883_OK=true` immediately after the strict SQLSTATE assertion succeeds, and the Makefile rollup must require both that marker and a `42883` substring in the live test log before success. Empirical PG16 evidence captured 2026-05-08 with `postgres:16-alpine@sha256:4e6e670bb069649261c9c18031f0aded7bb249a5b6664ddec29c013a89310d50`:
...
- **PG20-34:** `LIVE_PG20_STRICT_MARKER_COUNT=20` is the only valid count after PG20-25 adds `LIVE_PG20_NEGATIVE_SQLSTATE_42883_OK`. It includes the count record itself and excludes `PHASE20_ALL_PASS`.
...
- **PG20-37:** CI invokes the outer target as the checkout owner with `make live-pg-smoke`, not `sudo -E make live-pg-smoke`. The outer target performs only pre-sudo setup and locked-helper Git checks. CI sets `LIVE_PG20_USE_SUDO=1`, so runtime detection, SQL-shape preflight, live test execution, and marker verification run inside `_live-pg-smoke-inner` through `sudo -E` after those Git checks pass.
...
- **PG20-39:** Add non-strict timing markers: `LIVE_PG20_DETECT_OK=true`, `LIVE_PG20_DETECT_SLOW=true` when detection exceeds 2000ms on success, `LIVE_PG20_DETECT_MS`, `LIVE_PG20_CONTAINER_MS`, `LIVE_PG20_SEED_MS`, `LIVE_PG20_SUITE_DURATION_S`, `LIVE_PG20_WALL_CLOCK_BUDGET_S=180`, `LIVE_PG20_WALL_CLOCK_NEAR_BUDGET=true` when wall clock exceeds 162s, and `LIVE_PG20_WALL_CLOCK_OK=<true|false>`. Detection duration is hard-gated at `< 6000ms` on both success and no-runtime paths. Wall-clock budget is memory's 120s cold-start note multiplied by 1.5; wall clock is green at `<=180s`, emits a soft near-budget warning at `>162s`, and hard-fails above `216s` (`budget * 1.2`).
...
### r2 Narrow-Fold Decisions (r6 amended)
- **PG20-41:** Every runtime health subprocess is bounded to 5s through the shared Go runtime probe. The Makefile must not implement its own podman/docker shell fallback. It invokes `go -C dbee run ./cmd/probe-runtime` or a compiled equivalent, and that command uses parent `context.WithCancel`, two concurrent probes, child `context.WithTimeout(5*time.Second)`, and `exec.CommandContext`; bare `exec.Command(...).Run()` is forbidden. The command honors `LIVE_PG20_CONTAINER_PROVIDER` by validating only the requested provider when set. The probe emits exactly one machine-readable status line on stdout and no rollup markers. The line is pipe-separated key/value fields with keys `STATUS`, `PROVIDER`, `DURATION_MS`, and `DETAIL`; valid statuses are `ok`, `no_runtime`, and `internal_error`, and valid providers are `podman`, `docker`, and `none`. Exit codes are distinct: `0=ok`, `1=no_runtime`, `2=internal_error`; any other nonzero exit or malformed status line is treated as `internal_error`. The Makefile parser must validate the status line strictly: exactly one `STATUS` field, `STATUS` in the allowed set, `ok` requires `PROVIDER=podman|docker`, `no_runtime` requires `PROVIDER=none` plus nonempty `DETAIL`, and every path requires numeric `DURATION_MS < 6000`. Parse failure emits `LIVE_PG20_PROBE_INTERNAL_ERROR=true` and fails regardless of `LIVE_PG20_REQUIRED`. Makefile owns all marker emission, including no-runtime skip/fail markers. Probe result normalization is mandatory inside the status detail: `0=healthy`, `124=timeout after 5s`, `127=binary-not-found`, `>=128=signal-killed`, and any other nonzero status is `unhealthy`. Healthy podman plus hung docker must complete in `< 1500ms` and cancel the loser before its 5s timeout. If any Makefile child process is backgrounded, the recipe must use idempotent cleanup with a `_cleanup_done` guard, track only the currently running `child_pid`, capture and discard any cleanup `wait` status, explicitly `return 0`, and use split signal traps (`trap cleanup EXIT`; `trap 'cleanup; exit 130' INT`; `trap 'cleanup; exit 143' TERM`); the combined `trap 'cleanup' EXIT INT TERM` pattern is forbidden.
- **PG20-42:** No-runtime marker accounting is explicit and restored by ordering runtime detection before SQL-shape preflight. On no-runtime, emit only `LIVE_PG20_SKIPPED_NO_RUNTIME=true`, `LIVE_PG20_DETECT_MS=<ms>`, `PHASE20_ALL_PASS=false`, and the exact PG20-38 diagnostic message as output/rollup records; do not emit any strict success marker or `LIVE_PG20_STRICT_MARKER_COUNT`. The no-runtime detection duration must be parsed as numeric and asserted below 6000ms before the PG20-38 diagnostic or skip/fail markers are emitted; a missing, nonnumeric, or slow detection duration is an internal probe error, not a no-runtime skip. Local no-runtime skip may exit 0, but it is still a skip path and must not print success rollup evidence.
...
### r3 Narrow-Fold Decisions (r6 amended)
- **PG20-44:** CI fetches enough history for the locked-helper range check and keeps Git outside sudo. The `live-pg20-smoke` checkout uses `actions/checkout@v4` with `fetch-depth: 0`, or an explicit `git fetch origin master:refs/remotes/origin/master` before `make live-pg-smoke`. The outer Makefile target, running as the checkout owner, assigns `base="$$(git merge-base HEAD origin/master)"` and records the locked-helper result in `$(LIVE_PG20_ARTIFACT_ROOT)/locked-helpers-status` as `ok` or `tampered`; it must not emit `LIVE_PG20_LOCKED_HELPERS_UNTOUCHED_OK=true` before runtime detection. The outer target performs Git checks only before sudo. Runtime probe, SQL-shape preflight, live test, and rollup verification run inside `_live-pg-smoke-inner`, invoked directly by default or via `sudo -E make _live-pg-smoke-inner` when `LIVE_PG20_USE_SUDO=1`. `_live-pg-smoke-inner` must not invoke `git merge-base`, `git diff`, or any other Git command. After runtime detection succeeds, `_live-pg-smoke-inner` reads the sentinel: `ok` emits `LIVE_PG20_LOCKED_HELPERS_UNTOUCHED_OK=true`, and anything else fails before strict rollup. On no-runtime, the sentinel is ignored and the locked-helper marker is not emitted. The CI job must not call `sudo -E make live-pg-smoke`.
- **PG20-45:** Split SQL-shape evidence into two strict markers: `LIVE_PG20_SQL_SHAPE_PREFLIGHT_OK=true` after same-package SQL-constant preflight succeeds, and `LIVE_PG20_ROWS_FROM_LIVE_OK=true` after live `GetColumnsRich` exercises production ROWS FROM SQL successfully. With `LIVE_PG20_NEGATIVE_SQLSTATE_42883_OK`, strict count is `20`.
...
*Context revised: 2026-05-08 r6 narrow fold*
