...
- Migration failure is fatal for the current process: a central fatal latch blocks UI/editor/drawer/public accessors until restart, and an age-aware in-progress migration lock probe blocks handler initialization before `register()` and `m.core_loaded`.
...
- The legacy `"global"` namespace is retired. No picker, drawer, editor, or API path may create/read it after Phase 23; the drawer "global notes" master node is removed.
...
| F | `lua/dbee/notes_migration.lua` / `lua/dbee/api/state.lua` | One-time migration helper, pre-register migration-in-progress probe, and lazy-handler bootstrap hook. | Probes lock/sentinel before `register()` and `m.core_loaded`; runs actual migration after `Handler:new()` and before first UI/editor creation; central latch blocks accessors on fatal migration exception. |
| G | `lua/dbee/ui/editor/init.lua` | Startup default note selection and namespace wrapper delegation. | No automatic `global` welcome recreation; raw `global` and unauthorized `folder:*` creates are rejected; EditorUI delegates lifecycle helpers to `notes_namespace`. |
...
| K | `ci/headless/check_folder_scoped_notes.lua` | New Phase 23 behavior suite. | Emits 81 behavior/migration markers plus one diagnostic perf marker. |
...
58. `GN23_DRAWER_GLOBAL_MASTER_NODE_REMOVED_OK`
59. `GN23_PROBE_BEFORE_REGISTER_OK`
60. `GN23_PROBE_STALE_LOCK_AGES_OUT_OK`
61. `GN23_RECOVERY_VALIDATES_STAGING_ABSENT_AND_SIZE_OK`
62. `GN23_NOTES_DIR_BOOTSTRAP_OK`
63. `GN23_LATCH_CENTRAL_HELPER_GREP_GUARD_OK`
64. `GN23_CACHE_INVALIDATION_GATED_TO_MIGRATION_OK`
65. `GN23_NOTES_MIGRATION_INSPECT_COMMAND_OK`
66. `GN23_DELETE_FOLDER_VIA_EDITOR_ENTRY_OK`
67. `GN23_ERROR_KIND_VOCAB_LOCKED_OK`
68. `GN23_RECOVERY_PROMOTE_MANIFEST_TS_MATCH_OK`
69. `GN23_FOLDER_PREFIX_RESERVED_OK`
70. `GN23_RECOVERY_SENTINEL_AFTER_VALIDATION_OK`
71. `GN23_COLLISION_RESERVED_SET_PER_FOLDER_OK`
72. `GN23_MIGRATION_IN_PROGRESS_MESSAGE_SINGLE_SOURCE_OK`
73. `GN23_LATCH_FLAG_TABLE_DOCUMENTED_OK`
74. `GN23_INSPECT_BYPASSES_FATAL_LATCH_OK`
75. `GN23_INSPECT_BOUNDED_TRAVERSAL_OK`
76. `GN23_NAMESPACE_INPUT_VALIDATION_OK`
77. `GN23_LATCH_HELPERS_LEXICAL_BEFORE_CONSUMERS_OK`
78. `GN23_NOTES_NAMESPACE_PUBLIC_INTERNAL_SPLIT_OK`
79. `GN23_DRAWER_REMOVAL_USER_NOTIFY_OK`
80. `GN23_INSPECT_OUTPUT_FORMAT_LOCKED_OK`
81. `GN23_NAMESPACE_CLEAR_CACHE_INTERNAL_ANNOTATION_OK`
82. `GN23_FOLDER15_PRESERVED_OK`
83. `GN23_NOTES01_PICKER_CONTRACT_PRESERVED_OK`
84. `GN23_LOCKED_HELPERS_UNTOUCHED_OK`
85. `GN23_NO_GO_RPC_ADDED_OK`
...
| Owner | Count | Markers |
| --- | --- | --- |
| `ci/headless/check_folder_scoped_notes.lua` | 81 | GN23 behavior/migration markers 1-81 plus diagnostic perf marker. |
| `ci/headless/check_ux13_rollup.lua` | 4 | Preservation/guard markers 82-85 plus count/all-pass sentinels. |

## Plan-Gate r2 Concerns

- Migration must run from `lua/dbee/api/state.lua` with a pre-`register()` age-aware in-progress probe, then actual migration at the end of `setup_handler()` after `Handler:new()` returns and before the first caller can create `EditorUI`.
- Migration atomicity must use notes-dir bootstrap, load preconditions, load-uncertainty fail-closed checks, duplicate-folder-id guard, same-filesystem preflight, `vim.loop.fs_mkdir(path, 448)` lock acquisition, post-lock sentinel/recovery recheck, post-lock folder reload, staged clone verification, pre-recorded promote-manifest promotion with per-folder reserved destination sets, rollback of only migration-created files, recovery-manifest/promote-manifest validation before sentinel, final-path recovery-manifest handling, staging-absent/size/basename recovery validation, and fatal latch blocking.
...
- Folder deletion: drawer/actions route through `EditorUI:delete_folder_namespace(...)`, which delegates to notes namespace lifecycle and clears editor caches; no direct filesystem work in convert/model.
...
- Folder-scoped note creation routes through `notes_namespace` authority; raw `EditorUI:namespace_create_note("folder:<id>")` is blocked without an authority flag, and raw `"global"` namespace creation is always rejected.
- Promote manifest uses a pre-recorded manifest with two atomic writes total, not per-rename rewrite/read-back.
- Recovery-manifest deletes promote-manifest only when their run timestamp matches or recovery final paths are a superset of promote planned paths; otherwise both remain for inspection.
