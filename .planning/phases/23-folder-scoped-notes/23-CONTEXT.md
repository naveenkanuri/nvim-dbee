...
4. Empty folder lists under `loaded_ok` are valid fresh-user states. If every folder-capable source is `loaded_ok` and the total folder count is zero:
   ...
   - if `notes/global/` contains readable `*.sql` files, create the canonical backup, delete `notes/global/`, and write `.notes-migration-v1` without clones. Legacy notes survive in backup for manual user relocation after folders are created.
5. Folder IDs are checked for duplicates across all folder-capable sources before namespace creation. The same duplicate-folder-ID check runs at runtime lookup and namespace creation.
6. Before staging, remove stale `.notes-migration-v1.staging-<pid>-*` and `.notes-migration-v1.trash-<pid>-*` dirs whose pid is no longer alive or whose mtime is older than 1 hour.
7. Staging/trash dirs are same-filesystem siblings of final namespace dirs. Verify the invariant with `vim.loop.fs_statfs(notes_dir)` and a sibling precheck dir before creating staging dirs; abort on mismatch.
...
9. Verify staged file counts, then promote staged files into final `notes/folder:<folder_id>/` paths per file. Existing target filenames get `<stem>.global-migrated.sql`; final namespace dirs are created as needed. Track every migration-created final file in a manifest and roll back only those files on non-EXDEV promote failure.
...
11. Preserve a backup by renaming/copying legacy `notes/global/` to `notes/global.bak`, or `notes/global.bak.YYYYMMDDHHMMSS` if the default backup already exists. Backup creation failure logs a warning and does not invalidate a completed clone promotion.
...
13. Atomically write `notes/.notes-migration-v1` through a tmp file then rename. If sentinel write fails after migration is functionally complete, emit a warning and write `notes/.notes-migration-v1.recovery-needed` as a JSON manifest containing folder IDs, clone basenames/counts, backup path, global deletion state, and timestamp. The next launch recovery path validates the manifest before writing the missing sentinel.
...
- Migration is idempotent: if sentinel exists, do nothing; if `global/` is absent and sentinel missing, write sentinel after ensuring current folder namespace dirs exist. If `.notes-migration-v1.recovery-needed` exists, validate its manifest, write the missing sentinel, and clear the marker only when all listed clone files still exist.
...
- Namespace deletion should stage/rename the directory to `.notes-migration-v1.trash-<pid>-<random>/folder:<id>/` before source mutation, then finalize only after folder removal succeeds; restore the staged directory on handler failure. Cross-filesystem rename is unsupported and fails closed before source mutation.
