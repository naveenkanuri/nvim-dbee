...
**Gathered:** 2026-05-08
**Status:** Ready for planning r3 after r2 narrow fold
...
All decisions are **status: locked at r2** after the r2 narrow fold.

| ID | Status | Decision | Evidence / Rationale |
| --- | --- | --- | --- |
| ORA24-01 | locked at r2 | Rewrite scheme is percent-encoding-style: `$ -> _x24_`, `# -> _x23_`. The mapping is reversible by scanning rewritten names for those substrings. User names containing `_x24_` or `_x23_` in any case are rejected before rewrite. | Avoids adding non-word characters to driver identifiers and avoids reverse-map ambiguity. go-ora compares bind names case-insensitively, so sentinel checks must case-fold. |
| ORA24-02 | locked at r2 | Implement a small Oracle SQL tokenizer/scanner. It must skip single-quoted strings with `''`, q-quoted strings using the full ORA24-17 delimiter rule, double-quoted identifiers, line comments, and block comments. | Regex-only SQL rewriting would rewrite bind-looking text inside literals/comments. Oracle SQL docs define the q-quote alternate literal mechanism. |
| ORA24-03 | locked at r2 | In code regions only, detect bind references using `:([A-Za-z_][A-Za-z0-9_$#]*)`. Leading `$` and `#` are invalid and must fail user validation. | Phase 24 accepts Oracle-style `$` / `#` in the identifier body, not as the first character. Underscore-first remains allowed for compatibility with existing validator behavior. |
| ORA24-04 | locked at r2 | Apply rewriting at the Oracle adapter boundary before go-ora sees SQL or named args. Plain SQL uses rewritten text before `conn.ExecContext` / `d.c.QueryOnConn`; PL/SQL uses rewritten text before `conn.ExecContext`; user bind map keys are transformed before `sql.Named`. Driver-name collision detection is case-insensitive across SQL refs, bind-map keys, and cursor params. Fast-path eligibility must scan SQL text for case-folded `_X24_` / `_X23_` and validate code-region bind refs before returning unchanged. | Current plain SQL path passes `query` and `bindArgs` at `oracle_driver.go:474-509`; PL/SQL path passes `plsqlQuery` at `oracle_driver.go:540-560`; go-ora matches parameter names case-insensitively; SQL-side sentinel refs would bypass a `$#`-only fast path. |
| ORA24-05 | locked at r2 | Split validation into explicit surfaces and delete legacy `validateOracleBindName`. `validateOracleBindNameUser(name)` allows `[A-Za-z_][A-Za-z0-9_$#]*` but rejects reserved/unsafe names and case-insensitive sentinel substrings; `validateOracleBindNameDriver(name)` keeps strict go-ora-compatible `[A-Za-z_][A-Za-z0-9_]*` and reserved/unsafe protection. | Current `validateOracleBindName` at `oracle_driver.go:310-318` combines user and driver concerns and rejects `$` / `#`; keeping it would allow ambiguous call-site drift. |
| ORA24-06 | locked at r2 | Reverse-map go-ora errors mentioning rewritten names back to original names for colon-prefixed bind occurrences and known bare go-ora parameter-error templates only. Reverse mapping happens after `formatOracleError`, and wrappers preserve `Unwrap()` to the original driver error. | Naive substring replacement can corrupt incidental user/NLS text. Phase 22 formatting currently rebuilds error strings without preserving wrappers; go-ora emits bare parameter names in known templates. |
| ORA24-07 | locked at r2 | Cursor marker detection, broad validation, parse, and cleanup must use the same code-region span list as the rewriter. The locked pipeline is: tokenize once, scan code regions for cursor markers, validate with `validateOracleBindNameUser`, remove cursor marker comments only inside code regions, rewrite the cleaned SQL, then execute cleaned and rewritten SQL. | Current `cursorMarkerPattern` and `cursorMarkerBroadPattern` scan raw SQL. Phase 24 cannot leave cursor markers on a different lexical authority from bind rewriting, and r2 removes the clean-query/preserve-marker contradiction. |
| ORA24-08 | locked at r2 | DBMS_OUTPUT internal binds `:p_line` and `:p_status` are static internal names and do not need rewrite. Audit must assert they remain unchanged. | Phase 22 lockstep is covered by `oracle_driver_dbms_output_test.go`; `fetchDBMSOutputFromConn` uses `:p_line` / `:p_status` in `oracle_driver.go:574-576`. |
| ORA24-09 | locked at r2 | Preserve Phase 22 hotfix behavior that `:= /*CURSOR*/` and stray `/*CURSOR*/` comments are not treated as cursor markers. | Current broad regex excludes `=` at `oracle_refcursor.go:22-28`; `oracle_refcursor_test.go` has stray cursor comment coverage. |
| ORA24-10 | locked at r2 | Emit 14 ORA24 strict markers, one `PHASE24_ALL_PASS=true` rollup, and diagnostics: `ORA24_REWRITE_US_P50`, `ORA24_REWRITE_US_P95`, `ORA24_TOKENIZER_CASES_DIAGNOSTIC`, `ORA24_SENTINEL_CORPUS_DIAGNOSTIC`, and `LIVE_PG24_QQUOTE_UNSUPPORTED`. | Marker table in `PLAN.md` defines ownership and success criteria. |
| ORA24-11 | locked at r2 | Extend `oracle_bind_audit_test.go` so all dynamic `sql.Named` sites are proven to pass through explicit user/driver validation and/or the new rewrite helper. The audit must fail closed on unsupported computed first args and any reintroduction of `validateOracleBindName`. | Current AST audit recognizes `validateOracleBindName` dominance at `oracle_bind_audit_test.go:357-470`; that must become two-surface aware. |
| ORA24-12 | locked at r2 | No live Oracle DB is required. Use existing mock drivers plus tokenizer/rewriter unit tests. | Existing Phase 22 tests use mock drivers and source scanning; the new behavior can be verified before go-ora by inspecting rewritten SQL/args and wrapped errors. |
| ORA24-13 | locked at r2 | Implementation scope is eight files: the six base files plus `oracle_driver_context_test.go` and `oracle_refcursor_test.go`. | Local audit found existing tests at `oracle_driver_context_test.go:213-218` and `oracle_refcursor_test.go:94-99` that must change from reject to accept/rewrite. |
| ORA24-14 | locked at r2 | The three locked helpers remain untouched. | Same locked helper contract as Phases 22 and 23. |
| ORA24-15 | locked at r2 | Preserve Phase 22 and 22.5 contracts with exact validator error text: reserved names reject, SQL and bind-map sentinel collisions reject case-insensitively, `p_` internal binds remain safe, `oracleSafeBindSuggestion` remains valid against the new user validator and driver validator as applicable, and Phase 22 rollup helpers still pass under updated semantics. | Phase 22/22.5 shipped commits `e8b1b6d`, `31cd153`, and `988341f`; r1 locks message text to prevent drift; r2 closes SQL-side sentinel bypass. |
| ORA24-16 | locked at r2 | Sentinel-collision UX is locked: `oracle bind name %q contains internal sentinel '_x24_' or '_x23_' (case-insensitive); rename to avoid collision`. | Reversible encoding depends on rejecting literal `_x24_` and `_x23_` substrings under case folding. |
| ORA24-17 | locked at r2 | Q-quote support is intentionally narrow and fail-loud: supported opener `X` is a single ASCII byte that is not whitespace and not `'`; if `X` is one of `(`, `{`, `[`, `<`, terminator is the matched closer `)`, `}`, `]`, `>` followed by `'`; otherwise terminator is `X'`. Unsupported q-quote delimiters, including whitespace, single quote, and multibyte UTF-8 delimiters, reject before rewrite and emit `LIVE_PG24_QQUOTE_UNSUPPORTED=true` in the tokenizer diagnostic test. | r2 chose narrow + fail-loud over full Oracle q-quote support to avoid doubling tokenizer complexity for rare delimiter forms. Full single-quote and multibyte delimiter support is deferred to v1.5. |
| ORA24-18 | locked at r2 | Delete `validateOracleBindName`. All call sites must explicitly choose `validateOracleBindNameUser` or `validateOracleBindNameDriver`; audit fails on the legacy function name token. | A compatibility wrapper would preserve the exact ambiguity Phase 24 is removing. |
| ORA24-19 | locked at r2 | Reverse-error mapping is boundary-aware and manual-scan only. It handles colon-prefixed `:driver_name` occurrences with non-identifier trailing boundaries and a small known-template set for bare go-ora parameter names. Incidental bare substrings are never replaced, and production reverse mapping must not call `strings.NewReplacer`. | Prevents mangling user data or localized message text while still handling go-ora's bare `parameter %s is not defined...` errors. |
| ORA24-20 | locked at r2 | Rewrite performance budgets are strict: small cohort (~1KB, 0-2 binds) P95 <= 5 microseconds; medium (~10KB, 10 binds) P95 <= 50 microseconds; large (~100KB or 50+ binds) P95 <= 1 millisecond. Budget method collects 220 iterations, discards the first 20 as warmup, retains N=200 measured samples, uses P95 sorted index 190, repeats 5 runs, and gates on median P95. Fast path for unchanged SQL and unchanged bind map is zero-alloc. | Rewriting runs on every Oracle execution and must not become visible latency or allocation churn; r2 locks sample method to reduce CI variance. |
| ORA24-21 | locked at r2 | No regexp is allowed in the per-query rewrite path. The tokenizer and bind detector use manual byte scanning / ASCII helpers. Regex remains allowed in tests and non-hot audit helpers only. | Prevents repeated regexp allocation/search costs and keeps scanner behavior explicit. |
| ORA24-22 | locked at r2 | Case-folding contract: sentinel detection and driver-name collision detection use ASCII uppercase-folded names via `strings.ToUpper` on the ASCII-only bind grammar; rewrite preserves user-supplied case verbatim for emitted driver names; `filterCursorBindNames` continues comparing original user names case-insensitively; reverse-error map keys are case-sensitive driver names generated by the adapter. | Aligns with go-ora case-insensitive parameter matching while preserving user-visible names and Phase 22 cursor filtering behavior. |
| ORA24-23 | locked at r2 | Rewrite/tokenizer work must happen before `d.mu.Lock()`. The mutex protects session connection acquisition and driver execution only, not CPU-bound SQL scanning. | Current `QueryWithBinds` locks before bind processing; doing the new scan under lock would serialize unrelated Oracle executions. |
| ORA24-24 | locked at r2 | Bare-name reverse mapping is allowed only for known go-ora error templates: `parameter %s is not defined`, `bind %s invalid`, and `parameter "%s" not found`. Template matching runs before colon-prefix scanning and substitutes only the captured driver-name token. | go-ora emits bare parameter names at `command.go:1785`; colon-only mapping leaks rewritten names. |
| ORA24-25 | locked at r2 | Q-quote compatibility is narrow + fail-loud. Supported q-quotes must also pass a deterministic go-ora-parser parity test; unsupported delimiters or go-ora parser mismatches reject at user surface before rewrite and are documented as v1.5 backlog. | Mock SQL capture can false-pass q-quote shapes that go-ora's `refineSqlText` / `:(\w+)` parser mishandles. |
| ORA24-26 | locked at r2 | Tokenizer runs at most once per query/exec call and emits `[]codeRegion` via `scanOracleSQLCodeRegions(query)`. Bind detection, cursor detection, sentinel validation, and cleanup consume that same span list. | Avoids duplicated lexical authority and scan-twice costs on large PL/SQL blocks. |
| ORA24-27 | locked at r2 | REF CURSOR pipeline ordering is locked: scan code regions once, detect/validate cursor markers, remove marker comments only inside code regions, rewrite the cleaned SQL, execute cleaned + rewritten SQL, and preserve skipped-region marker text unchanged. | Resolves the r2 contradiction between clean-query and marker-preservation wording. |
...
| REF CURSOR path | Parses strict cursor params, validates with go-ora grammar, passes `sql.Named(param, sql.Out{...})`. | Uses the shared code-region span list to parse original cursor params, remove only code-region marker comments, rewrite cursor param names for driver OUT args, execute cleaned + rewritten SQL, and keep original cursor names for result labeling/error text. |
...
- Full Oracle q-quote delimiter coverage for single-quote and multibyte delimiters; Phase 24 rejects those fail-closed and tracks them for v1.5.
