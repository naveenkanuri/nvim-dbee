...
`lsp.workspace_symbols = false` to suppress those capabilities. Code actions
are available through `vim.lsp.buf.code_action()` and can expand cached
single-table `SELECT *`, qualify unqualified table references, refresh the
schema cache, or reload table metadata. Set `lsp.code_actions = false` to
disable the whole code-action surface, or use the per-action flags shown above
for narrower rollback. Hover, resolve, workspace symbols, and code actions use
already-loaded schema cache data only, so details may be minimal until table
columns have warmed. Document symbols are source-text based and show SQL
references from the current buffer.
